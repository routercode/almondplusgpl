...
PATCH_DIR?=./patches
PATCH_CUSTOM_BOARD_DIR ?= ./custom-board-patches$(if $(wildcard ./custom-board-patches-$(KERNEL_PATCHVER)),-$(KERNEL_PATCHVER))
PATCH_OPENWRT_SPECIFIC_DIR ?= ./openwrt-specific-patches$(if $(wildcard ./openwrt-specific-patches-$(KERNEL_PATCHVER)),-$(KERNEL_PATCHVER))
FILES_DIR?=./files
...
define PatchDir/Quilt
	...
	@for patch in $$$$( (cd "$(2)" && if [ -f series ]; then $(call filter_series,series); else ls | grep -v README.txt | sort; fi; ) 2>/dev/null ); do ( \
...
define Kernel/Patch/Files
	$(if $(kernel_files),$(CCP) $(kernel_files) $(LINUX_DIR)/)
endef
define Kernel/Patch/Default
	rm -rf $(PKG_BUILD_DIR)/patches; mkdir -p $(PKG_BUILD_DIR)/patches
	$(call PatchDir,$(PKG_BUILD_DIR),$(PATCH_DIR),platform/)
	$(call PatchDir,$(PKG_BUILD_DIR),$(GENERIC_PATCH_DIR),generic/)
	$(call PatchDir,$(PKG_BUILD_DIR),$(PATCH_CUSTOM_BOARD_DIR),platform-custom-board/)
	$(call PatchDir,$(PKG_BUILD_DIR),$(PATCH_OPENWRT_SPECIFIC_DIR),platform-openwrt-specific/)
endef
