...
//
// $RCSfile: DP_TX_DRV.c,v $
// $Revision: 1.3 $
// $Author: jhsu $
// $Date: 2011/10/19 09:07:05 $
//
// ---------------------------------------------------------------------------
// >>>>>>>>>>>>>>>>>>>>>>>>> COPYRIGHT NOTICE <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
// ---------------------------------------------------------------------------
...
void DP_TX_Int_Process(void)
{
	...
	if (dp_tx_system_state == DP_TX_WAIT_HOTPLUG) {
		debug_printf("<%s:%d> state=DP_TX_WAIT_HOTPLUG\n", __func__, __LINE__);
		...
		//debug_printf("DP_COMMON_INT_STATUS4 = %.2x\n", (WORD)c);
		//DP_TX_Int_Info(c, 2);
		...
		/*
		   ...
		   {
		   debug_puts("Video format changed");
...
void DP_TX_Int_Info(BYTE int_v, BYTE id)
{
    switch(id)
    {
        case 0:
            //if(int_v & 0x80)
                //debug_puts("ENC_EN changed");
            //if(int_v & 0x40)
                //debug_puts("HDCP_Link changed");
            if(int_v & 0x08)
                debug_puts("BKSV ready");
            if(int_v & 0x04)
                debug_puts("SHA done");
            if(int_v & 0x02)
                debug_puts("AUTH changed");
            if(int_v & 0x01)
                debug_puts("AUTH done");
            break;
        case 1:
            if(int_v & 0x40)
                debug_puts("INT_HPD");
...
void DP_TX_HPD_Int_Handler(BYTE hpd_source)
{
	...
	debug_printf("<%s:%d> interrupt handling (dp_tx_system_state=%d, hpd_source=%d)\n", __func__, __LINE__, (WORD)dp_tx_system_state, hpd_source);
	if (dp_tx_system_state == DP_TX_INITIAL)
		return;
		...
		case 0:
			...
			if (IRQ_Vector & 0x04)	//HDCP IRQ
			{
				...
				if (c1 & 0x04)	//link intergrity failure
				{
					   ...
					   && (dp_tx_system_state != DP_TX_PARSE_EDID) && (dp_tx_system_state != DP_TX_CONFIG_VIDEO)) */
					{
						...
						debug_puts("HPD:____HDCP Sync lost!");
					}
				}
			}
			if (IRQ_Vector & 0x02)
			...
			if (test_irq) {
				...
				if (test_vector & 0x01) {
					...
					if (dp_tx_test_lane_count == 1) {
						...
						test_lt = 0;
						debug_puts("Test LC reduce w/o LT");
			...
			{
				if ((al & 0x01) == 0)
					debug_printf("AL lost\n");
				if (sl_cr == 0)
					debug_printf("CR lost\n");
				if (test_lt == 1)
					debug_printf("Test LT requested\n");
				   ...
				   && (dp_tx_system_state != DP_TX_LINK_TRAINING)) */
				{
					...
					DP_TX_Set_System_State(DP_TX_CONFIG_VIDEO);
					debug_puts("IRQ:____re-LT request!");
			...
			break;
		case 1:
			//debug_puts("HPD:_____plug!");
			debug_printf("<%s:%d> HPD:_____plug!\n", __func__, __LINE__);
			dp_tx_link_config_done = 0;
		...
		case 2:
			...
			if (c & DP_TX_INT_STATUS1_HPD)	//HPD detected
				...
				if (c & DP_TX_SYS_CTRL3_HPD_STATUS)
					...
					//hdcp_rx_repeater = 0;
					debug_printf("<%s:%d> HPD:_____HPD_Lost!\n", __func__, __LINE__);
					//debug_puts("HPD:_____HPD_Lost!");
					DP_TX_Set_System_State(DP_TX_WAIT_HOTPLUG);
				}
			}
...
void HDCP_Auth_Done_Interrupt()
{
	...
	if (c & DP_TX_HDCP_AUTH_PASS) {
		...
		//debug_puts("INT:Auth_Done = Fail");
		debug_puts("***auth_failed***");
		hdcp_auth_pass = 0;
		hdcp_auth_fail_counter++;
		if (hdcp_auth_fail_counter >= DP_TX_HDCP_FAIL_THRESHOLD) {
			debug_puts("HDCP_FAIL_THRESHOLD reached. ");
			hdcp_auth_fail_counter = 0;
...
void HDCP_Auth_Change_Interrupt()
{
	...
	else {						//debug_puts("INT:Auth_CHG = Fail");
		if (!send_blue_screen) {
			DP_TX_Blue_Screen_Enable();
		}
		if (avmute_enable) {
...
void DP_TX_Video_Changed_Handler(void)
{
	...
	if (((h_res_bak - 10) > h_res) || ((h_res_bak + 10) < h_res)) {
		...
		//debug_puts("h_res chg");
		debug_printf("h_res_bak = %.2x, h_res = %.2x\n", (WORD)h_res_bak, (WORD)h_res);
	}
	if (((v_res_bak - 10) > v_res) || ((v_res_bak + 10) < v_res)) {
		video_changed = 1;

		//debug_puts("v_res chg");
		debug_printf("v_res_bak = %.2x, v_res = %.2x\n", (WORD)v_res_bak, (WORD)v_res);
	}
	if (((h_act_bak - 10) > h_act) || ((h_act_bak + 10) < h_act)) {
		video_changed = 1;

		//debug_puts("h_act chg");
		debug_printf("h_act_bak = %.2x, h_act = %.2x\n", (WORD)h_act_bak, (WORD)h_act);
	}
	if (((v_act_bak - 10) > v_act) || ((v_act_bak + 10) < v_act)) {
		video_changed = 1;

		//debug_puts("v_act chg");
		debug_printf("v_act_bak = %.2x, v_act = %.2x\n", (WORD)v_act_bak, (WORD)v_act);
	}
	if (I_or_P_bak != I_or_P) {
		video_changed = 1;

		debug_puts("I_or_P chg");
	}
	if (video_changed) {
		debug_puts("video chg!");
		video_config_done = 0;
		if (mode_dp) {
			DP_TX_Set_System_State(DP_TX_CONFIG_VIDEO);
			DP_TX_Video_Disable();
	...
	//delay_ms(300);//wait for HDMI RX stable
	h_res_bak = h_res;
	v_res_bak = v_res;
	h_act_bak = h_act;
	v_act_bak = v_act;
...
void DP_TX_PLL_Changed_Int_Handler(void)
{
                ...
                && (dp_tx_system_state != DP_TX_PARSE_EDID))
    {
        ...
        {
            debug_puts("PLL:_____PLL not lock!");
...
BYTE Sink_Is_DP_HDMI(void)
{
	...
	/*
	   ...
	   debug_puts("unknown device attached");
	   return 1;
	   } */
}
...
void DP_TX_Initialization(void)
{
	...
	debug_printf("<%s:%d> DP_TX_initing (dp_tx_system_state=%x)\n", __func__, __LINE__, dp_tx_system_state);
	//video_bpc = 8;
	DP_TX_Variable_Init();
	DP_TX_Read_Reg(HDMI_TX_PORT0_ADDR, DP_POWERD_CTRL_REG, &c);
...
void DP_TX_Set_System_State(BYTE ss)
{
	...
	case DP_TX_WAIT_HOTPLUG:
		...
		//debug_puts("WAIT_HOTPLUG");
		debug_printf("<%s:%d> to WAIT_HOTPLUG\n", __func__, __LINE__);
		break;
	case DP_TX_PARSE_EDID:
		dp_tx_system_state = DP_TX_PARSE_EDID;

		//debug_puts("PARSE_EDID");
		debug_printf("<%s:%d> to PARSE_EDID\n", __func__, __LINE__);
		break;
	case DP_TX_CONFIG_VIDEO:
		dp_tx_system_state = DP_TX_CONFIG_VIDEO;
		video_config_done = 0;

		//debug_puts("CONFIG_VIDEO");
		debug_printf("<%s:%d> to CONFIG_VIDEO\n", __func__, __LINE__);
		break;
	case DP_TX_CONFIG_AUDIO:
		dp_tx_system_state = DP_TX_CONFIG_AUDIO;

		//debug_puts("CONFIG_AUDIO");
		debug_printf("<%s:%d> to CONFIG_AUDIO\n", __func__, __LINE__);
		break;
	case DP_TX_LINK_TRAINING:
		dp_tx_system_state = DP_TX_LINK_TRAINING;
		dp_tx_link_config_done = 0;

		//debug_puts("LINK_TRAINING");
		debug_printf("<%s:%d> to LINK_TRAINING\n", __func__, __LINE__);
		break;
	case DP_TX_HDCP_AUTHENTICATION:
		dp_tx_system_state = DP_TX_HDCP_AUTHENTICATION;

		//debug_puts("HDCP_AUTH");
		debug_printf("<%s:%d> to HDCP_AUTH\n", __func__, __LINE__);
		break;
	case DP_TX_PLAY_BACK:
		dp_tx_system_state = DP_TX_PLAY_BACK;

		//debug_puts("PLAY_BACK");
		debug_printf("<%s:%d> to PLAY_BACK\n", __func__, __LINE__);
		break;
	default:
		break;
	}
...
void DP_TX_Timer_Slot1(void)
{
	...
	if (dp_tx_system_state == DP_TX_PARSE_EDID) {
		debug_printf("<%s:%d> state=DP_TX_PARSE_EDID\n", __func__, __LINE__);
		if (mode_dp) {
			debug_printf("<%s:%d> in dp mode\n", __func__, __LINE__);
			edid_break = 0;
			DP_TX_LL_CTS_Test();
			if (edid_break) {
				dp_tx_edid_err_code = 0xff;
				debug_puts(" edid corruption");
			}
			//DP_TX_Set_System_State(DP_TX_LINK_TRAINING);
			DP_TX_Set_System_State(DP_TX_CONFIG_VIDEO);
		}
...
void DP_TX_Timer_Slot2(void)
{
	...
	//BYTE c;//i,a,j;
	debug_printf("<%s:%d> DP_TX_Timer_Slot2 (dp_tx_system_state=%d)\n", __func__, __LINE__, dp_tx_system_state);
...
{
	debug_printf("DP_TX_Timer_Slot3\n");
	if (dp_tx_system_state == DP_TX_PLAY_BACK)
...
void DP_TX_Config_Video(void)
{
	...
	if (!video_config_done) {
		...
		/*
		   ...
		   debug_printf("max_bw = %.2x\n",(WORD)dp_tx_bw);
		   debug_printf("max_lc = %.2x\n",(WORD)dp_tx_lane_count);
		   }              
		 */
		if (BIST_EN) {
			debug_printf("<%s:%d> BIST_EN\n", __func__, __LINE__);
			if (!SWITCH4)
				bForceSelIndex = 0;
	...
	anx9805_dump_reg(HDMI_TX_PORT0_ADDR, DP_TX_VIDEO_BIT_CTRL_0_REG, 1);
		...
		else					//HDMI MODE
		{
			...
			anx9805_dump_reg(HDMI_TX_PORT0_ADDR, DP_TX_VIDEO_BIT_CTRL_0_REG, 1);
			DP_TX_Read_Reg(HDMI_TX_PORT1_ADDR, HDMI_SYS_STATUS, &c);
			debug_printf("<%s:%d> hdmi mode [7a:01=%02x]\n", __func__, __LINE__, (WORD)c);
	...
	if (!mode_dp) {
		...
		if ((c & HDMI_VID_STATUS_VID_STABLE) != 0x01) {
			debug_printf("Vid not stable [7a:02]=%02x\n", (WORD)c);
			return;
		}
		DP_TX_Clean_HDCP();
...
BYTE HDMI_TX_Config_Packet(void)
{
	...
	if (info_packet_sel != 0x00) {
		...
		if (info_packet_sel & HDMI_TX_avi_sel) {
			...
			debug_puts("HDMI____config avi infoframe packet."); 
		...
		if (info_packet_sel & HDMI_TX_audio_sel) {
			debug_printf("<%s:%d> audio infoframe\n", __func__, __LINE__);

			debug_puts("HDMI____config audio infoframe packet.");
...
BYTE HDMI_TX_Load_Infoframe(HDMI_PACKET_TYPE member, HDMI_INFOFRAME_STRUCT * p)
{
	...
	for (i = 0; i <= p->length; i++) {
		...
		DP_TX_Read_Reg(HDMI_TX_PORT0_ADDR, address[member]+3+i, &c);
		  debug_printf("HDMI____pb-byte = 0x%.2x\n",(WORD) c);
	}
	return exe_result;
}
...
void HDMI_TX_RepeatTimes_Setting(void)
{
	...
	debug_printf("hdmi_tx_tx_pix_rpt: %.2x\n", (WORD)hdmi_tx_tx_pix_rpt);
	debug_printf("hdmi_tx_video_id: %.2x\n", (WORD)hdmi_tx_video_id);
	...
	debug_printf("   HDMI_VID_CTRL2 1 is  %.2x \n", (WORD)c);
}
...
void DP_TX_Link_Training(void)
{
	...
	if (c & 0x70) {
		c = (c & 0x70) >> 4;
		debug_printf("HW LT failed, ERR code = %.2x\n", (WORD) c);
...
/*
    if(c & 0x70)
    {
		...
		//if(c == 0x01)
		{
			//debug_puts("Much deff error!");
			if(dp_tx_bw == 0x0a)
			{
				debug_puts("Force to RBR");
				DP_TX_RST_AUX();
				dp_tx_bw = 0x06;
				DP_TX_HW_LT(dp_tx_bw, dp_tx_lane_count);
			}
        ...
        switch(colordepth_packet_mode)
           {
            case 0x00:
              ...
              debug_printf("ColorDepth not indicated\n");
              break;
            case 0x24:
              DP_TX_Read_Reg(HDMI_TX_PORT1_ADDR, HDMI_GNRL_CTRL_PKT_REG  , &c);
              DP_TX_Write_Reg(HDMI_TX_PORT1_ADDR, HDMI_GNRL_CTRL_PKT_REG , c  & 0xa3 );   
              ...
              debug_printf("ColorDepth 24 bits per pixe\nl");
              break;
            case 0x30:
              DP_TX_Read_Reg(HDMI_TX_PORT1_ADDR, HDMI_GNRL_CTRL_PKT_REG  , &c);
              DP_TX_Write_Reg(HDMI_TX_PORT1_ADDR, HDMI_GNRL_CTRL_PKT_REG , c  & 0xa3 );   
              ...
              debug_printf("ColorDepth 30 bits per pixel\n");
              break;
            case 0x36:
              DP_TX_Read_Reg(HDMI_TX_PORT1_ADDR, HDMI_GNRL_CTRL_PKT_REG  , &c);
              DP_TX_Write_Reg(HDMI_TX_PORT1_ADDR, HDMI_GNRL_CTRL_PKT_REG , c  & 0xa3 );   
              ...
              debug_printf("ColorDepth 36 bits per pixel\n");
              break;
            }
	...
	if (dp_tx_hdcp_enable) {
		if (mode_dp) {
			if (!dp_tx_hdcp_capable_chk) {
				...
				c = ByteBuf[0];
				if (!(c & 0x01)) {
					debug_puts("RX not capable HDCP");
					return;
				}
				if ((ByteBuf[0] & 0x02) == 0x02)	// for HDCP cascade.cguo
				{
					...
					//hdcp_rx_repeater = 1; 
					debug_puts("DP downstream is a repeater");
				}
		...
		if (!dp_tx_hw_hdcp_en) {
			...
			/*
			   if(hdcp_rx_repeater)
			   {
			   //debug_puts("Waiting HDCP repeater...");
			   for (i = 0; i < 1400; i++)
			   {
...
{
	BYTE c;
	debug_printf("<%s:%d> HDCP Function Reset\n", __func__, __LINE__);
	//debug_puts("HDCP Function Reset");
...
void DP_TX_Show_Infomation(void)
{
	...
	if (mode_dp)				//dp
	{
		...
		if (dp_tx_ssc_enable)
			...
			debug_puts("   SSC Off");
		debug_printf("   M = %lu, N = %lu, PCLK = %.4f MHz\n", M_val, N_val, pclk);
	}

	else {
		if (mode_hdmi)
			debug_puts("   HDMI Mode");

		else
			debug_puts("   DVI Mode");
	}
	Video_Timing();
	debug_printf("   Total  %d * %d \n", h_res, v_res);
	debug_printf("   Active  %d * %d ", h_act, v_act);
	if (mode_dp) {
		fresh_rate = pclk * 1000;
		fresh_rate = fresh_rate / h_res;
		fresh_rate = fresh_rate * 1000;
		fresh_rate = fresh_rate / v_res;
		debug_printf(" @ %.2fHz", fresh_rate);
	}
	DP_TX_Read_Reg(HDMI_TX_PORT0_ADDR, DP_TX_VID_STATUS, &c);
	if (I_or_P)
		debug_printf("\n   Interlace, ");
	...
	//debug_puts("Type cmd timing to set any video timing!");
...
{
    BYTE c,c1,i;
    debug_puts("Parse EDID begins......");
    DP_TX_Read_Reg(EDID_Dev_ADDR, 0x00, &c);
	debug_printf("edid[0] = %.2x\n", (WORD)c);
    delay_ms(2);
    DP_TX_Read_Reg(EDID_Dev_ADDR, 0x07, &c1);
	debug_printf("edid[7] = %.2x\n", (WORD)c);
    delay_ms(2);
    if((c == 0x00) && (c1 == 0x00))
    {
        for(i = 1; i < 7; i ++)
...
BYTE DP_TX_Read_EDID(void)
{
    ...
    delay_ms(2);
    //debug_printf("edid extension block = %.2x\n", (WORD)c);
    //DP_TX_Read_Reg(EDID_Dev_ADDR, 0, &c);
    //debug_printf("EDID[0] = 0x%.2x\n",(WORD)c);
    edid_length = 128 * (c + 1);
    if(edid_length <= 256)
    {
        for(i = 0; i < edid_length; i ++)
        {
            ...
            if(i < 0x7f)
                checksum = checksum + c;
            //debug_printf("checksum = 0x%.2x\n",(WORD)checksum);
            if(i == 0x7f)
            {
                checksum = ~checksum + 1;
                //debug_printf("checksum = 0x%.2x, edid[0x7f] = 0x%.2x\n",(WORD)checksum, (WORD)c);
                if(checksum != c)
...
/*BYTE DP_TX_AUX_EDIDRead_Byte(BYTE addrh, BYTE addrm, BYTE addrl, BYTE data1)
{
   ...
   DP_TX_Write_Reg(DP_TX_PORT0_ADDR, DP_TX_AUX_CTRL_REG, 0x04);    //read 0x15     //write  0x14
    ...
    debug_puts("DP_TX_AUX_DPCDRead_Byte() success.");
...
void DP_TX_BIST_Format_Config(WORD dp_tx_bist_select_number)
{
	...
	//else
	{
		debug_puts("config vid timing");
		if (!Force_Video_Resolution) {

			//debug_printf("dp_tx_edid_err_code= %.2x\n",(WORD)dp_tx_edid_err_code);
			...
			if ((dp_tx_edid_err_code == 0) && (edid_pclk_out_of_range == 0))	//commented for QDI test
			{
				...
				//debug_printf("mode dp or hdmi = %.2x\n",(WORD)mode_dp_or_hdmi);
...
void DP_TX_Config_Audio(void)
{
	...
	if (!SWITCH1) {
		...
		DP_TX_Read_Reg(HDMI_TX_PORT0_ADDR, SPDIF_AUDIO_STATUS0, &c);
		if ((c & 0x81) != 0x81) {
			debug_printf("SPDIF_AUDIO_STATUS0 = %.2x\n", (WORD) c);
		}
...
void DP_TX_EDID_Read(void)
{
	...
	debug_printf("EDID error code = %.2x\n",(WORD)dp_tx_edid_err_code);
	...
	if (ByteBuf[0] & 0x04)		//test edid
	{
		...
		//if(c&0x04)
		{
			...
			/*c = DP_TX_AUX_DPCDRead_Byte(0x00, 0x02,0x61);
			   ...
			   debug_printf("check sum write value = %.2x\n", (WORD)c); */
			ByteBuf[0] = 0x04;
			DP_TX_AUX_DPCDWrite_Bytes(0x000260, 1, ByteBuf);
			...
			/*c = DP_TX_AUX_DPCDRead_Byte(0x00, 0x02,0x60);
			   ...
			   debug_printf("check_sum_write flag = %.2x\n", (WORD)c); */
		}
...
#ifdef CR_LOOP
		debug_puts("CLOCK_RECOVERY_PROCESS\n");
...
#ifdef CR_LOOP
		debug_puts("EQ_TRAINING_PROCESS");
		debug_puts("PT2 set");
...
#ifdef CR_LOOP
		debug_puts("LINKTRAINING_FINISHED");
...
#ifdef CR_LOOP
			debug_puts("CR training succeed\n");
...
#ifdef CR_LOOP
				debug_puts("CR training failed due to loop > 5");
...
#ifdef CR_LOOP
					debug_puts("Set PT0 to terminate LT");

#endif /*  */

#ifdef SW_LT_DEBUG
					debug_puts("CR failed, loop > 5");
...
#ifdef CR_LOOP
		debug_printf("Reading lane status: bLane0_1_Status = %.2x\n", (WORD) bLane0_1_Status);
...
#ifdef SW_LT_DEBUG
						debug_puts("EQ training failed due to loop > 5");
...
//#ifdef SW_LT_DEBUG
				debug_puts("SW LT success!\n");
				DP_TX_Read_Reg(DP_TX_PORT0_ADDR, DP_TX_LINK_BW_SET_REG, &c0);
				debug_printf("dp_tx_final_bw = %.2x\n", (WORD) c0);
				DP_TX_Read_Reg(DP_TX_PORT0_ADDR, DP_TX_LANE_COUNT_SET_REG, &dp_tx_final_lane_count);
				debug_printf("dp_tx_final_lane_count = %.2x\n", (WORD) dp_tx_final_lane_count);
...
#ifdef SW_LT_DEBUG
			debug_puts("EQ training failed due to CR loss");
...
#ifdef SW_LT_DEBUG
				debug_puts("EQ training succeed!\n");
...
#ifdef SW_LT_DEBUG
		debug_printf("########### Reading DPCD_LANE0_1_STATUS = %.2x\n", (WORD) bLane0_1_Status);
...
#ifdef SW_LT_DEBUG
			debug_puts("3write pt0");
			debug_puts("Set PT0 to terminate LT");

#endif /*  */
#ifdef SW_LT_DEBUG
			debug_puts("LT failed2!\n");
...
void HDMI_TX_CSCandColorDepth_Setting(void)
{
	...
	if ((Hdmi_Edid_RGB30bit == 0) && (Hdmi_Edid_RGB36bit == 0) && (Hdmi_Edid_RGB48bit == 0))	//not deep color mode
	{
		...
		debug_printf("<%s:%d> old [72:0a]=%02x\n", __func__, __LINE__, (WORD) c);
		c &= 0xf1;
		...
		debug_printf("<%s:%d> I write %02x\n", __func__, __LINE__, (WORD) c);

		DP_TX_Read_Reg(HDMI_TX_PORT0_ADDR, DP_TX_VID_CTRL3_REG, &c);	// 72:0a
		debug_printf("<%s:%d> new [72:0a]=%02x\n", __func__, __LINE__, (WORD) c);

		//set 24 bit mode
		debug_printf("<%s:%d> (24bit mode)\n", __func__, __LINE__);
		...
		debug_printf("<%s:%d> (reset 16bit mode)\n", __func__, __LINE__);
		DP_TX_Read_Reg(HDMI_TX_PORT0_ADDR, DP_TX_VID_CTRL2_REG, &c);	// 72:09
	...
	{
		debug_printf("<%s:%d> (deep color mode)\n", __func__, __LINE__);
...
BYTE HDMI_Parse_EDIDHeader(void)
{
	...
	debug_printf("<%s:%d> Assume good header\n", __func__, __LINE__);
	HDMI_InitDDC_Read();
	return 1;
#else
	debug_printf("<%s:%d>\n", __func__, __LINE__);
	...
	if (temp == 0x01) {
		debug_puts("Bad header");
		dp_tx_edid_err_code = 0x01;
		return 0;
	}

	else {
		debug_puts("Good header");
		return 1;
	}
#endif
}
...
#ifdef	QUICK_BOOT_NO_CHECK
	debug_printf("<%s:%d>: bypass it, assume checksum good.\n", __func__, __LINE__);
#else
	//debug_printf("<%s:%d>: wait a while, take a cup of coffee\n", __func__, __LINE__);
	chk_sum = 0;
	for (i = 0; i < 127; i++) {
