...
struct etherip_tunnel {
 ...
 DEFINE_RWLOCK(etherip_lock);

 void etherip_tunnel_setup(struct net_device *dev);

/* add a tunnel to the hash */
 void etherip_tunnel_add(struct etherip_tunnel *tun)
{
...
/* delete a tunnel from the hash*/
 void etherip_tunnel_del(struct etherip_tunnel *tun)
{
	list_del(&tun->list);
}

/* find a tunnel in the hash by parameters from userspace */
 struct etherip_tunnel* etherip_tunnel_find(struct ip_tunnel_parm *p)
{
...
/* find a tunnel by its destination address */
 struct etherip_tunnel* etherip_tunnel_locate(u32 remote)
{
	struct etherip_tunnel *ret;
 ...
 int etherip_change_mtu(struct net_device *dev, int new_mtu)
{
 ...
 * packet */
 int etherip_tunnel_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct etherip_tunnel *tunnel = netdev_priv(dev);
	struct rtable *rt;
	struct iphdr *iph;
...
/* checks parameters the driver gets from userspace */
 int etherip_param_check(struct ip_tunnel_parm *p)
{
	if (p->iph.version != 4 ||
 ...
 * information */
 int etherip_tunnel_ioctl(struct net_device *dev, struct ifreq *ifr,
		int cmd)
{
	int err = 0;
...
out:
 ...
 const struct net_device_ops etherip_netdev_ops = {
...
accept:
 ...
 struct net_protocol etherip_protocol = {
	.handler      = etherip_rcv,
	.err_handler  = 0,
	.no_policy    = 0,
};
...
/* destroy all tunnels */
 void __exit etherip_destroy_tunnels(void)
{
	int i;
