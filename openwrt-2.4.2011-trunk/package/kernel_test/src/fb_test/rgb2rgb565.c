 ...
 *
 * Description:
 *   This a a raw picture converter that converts the standrad.raw to rgb565.raw.
 *
 * usage:
 *   rgb2rgb565 <srcrgb.raw> <desrgb565.raw>
...
// RGB565 using 2 bytes only per pixel
    desfile_size = offset_to_raw + rgb_size/3*2;
    ...
    for (cnt=0; cnt<rgb_size_div3; cnt++) {
        unsigned short r,g,b, rgb=0;

        r = src_buffer[offset_to_raw + rgb_cnt++] >> 3;
        g = src_buffer[offset_to_raw + rgb_cnt++] >> 2;
        b = src_buffer[offset_to_raw + rgb_cnt++] >> 3;
        rgb = (b << 11) + (g << 5) + r;
        des_buffer[offset_to_raw + argb_cnt++] = (unsigned char)rgb;
        des_buffer[offset_to_raw + argb_cnt++] = (unsigned char)(rgb >> 8);
    }
...
int main(int argc, char *argv[])
{
    ...
    if (3 == argc) {
        ...
        printf("usage:\n");
        printf("rgb2rgb565 <srcrgb.raw> <desrgb565.raw>\n");
        return 0;
    }
