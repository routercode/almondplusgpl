...
#################################
if [ "$TEST_Module" = "basic" ]; then

./nat-accept-all.sh

echo "run basic test and setup firewall policy into accept all "

elif [ "$TEST_Module" = "nat" ]; then

./nat-full-cone-port-restricted.sh
echo "run nat/arp/icmp/forwarding testing"

elif [ "$TEST_Module" = "virtual-server" ]; then
sleep 2
./virtual-server.sh

echo "run virtual server testing"

else 
./nat-accept-all.sh
echo "run other testing"
fi
