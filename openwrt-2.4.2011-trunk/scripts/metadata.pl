	...
	while (<FILE>) {
		...
		/^Kernel-Config:\s*(.+?)\s*$/ and do {
			...
			foreach my $config (@config) {
				...
				if ( !defined( $config{"CONFIG_PACKAGE_$package"} ) ) {
				    # $config may have been set by another kmod package!  Only set to 'n'
				    # if not already set.
				    if ( ! $override ) {
					...
					# Take the value user set in menuconfig for the kmod
					$kconfig{$config} = $config{"CONFIG_PACKAGE_$package"};
				    }
				}
=perl				
                                # This original code appears broken.  Note that $config will never be 'n'
                                # for one thing.  Anyway, this code always led to 'm' even if kmod was 'y'.
...
sub print_package_config_category($) {
	...
	foreach my $menu (@menus) {
		...
		foreach my $pkg (@pkgs) {
			...
			print "\t\tdefault m if DEFAULT_".$pkg->{name}."-m\n" if ( $pkg->{name} =~ /^kmod-/ );
