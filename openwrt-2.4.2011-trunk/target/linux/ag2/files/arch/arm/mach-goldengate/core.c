			   ...
			   struct clock_event_device *clk)
{
	...
	case CLOCK_EVT_MODE_PERIODIC:
		reload = (sys_clk_info.apb_clk / 1000000) * mSEC_10;
	...
	default:
		ctrl = 0;
		if(remove_timer_irq>1)
			remove_irq(tmr0_irq, &goldengate_timer_irq);
		remove_timer_irq++;
		break;
	}
...
static void __init goldengate_clocksource_init(void)
{
	...
	clocksource_goldengate.mult =
	    clocksource_khz2mult(APB_CLOCK / 1000,
		clocksource_goldengate.shift);
	clocksource_register(&clocksource_goldengate);
}
...
/*
 * Setup hardware acceleration jump table
 */
void cs_hw_accel_nop(void)
{
}

#include "cs_jt_init.c" //Suresh commented
