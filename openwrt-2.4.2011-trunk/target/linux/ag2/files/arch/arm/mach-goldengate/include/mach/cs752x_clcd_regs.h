...
//    LCDIMSC_REG          =  0x1c,    /* Interrupt Mask Set/Clear Register */
    #define MBERRORIM   (1 << 4)	// AHB master error interrupt enable
    #define VcompIM     (1 << 3)	// Vertical compare interrupt enable
    #define LNBUIM      (1 << 2)	// LCD next base address update interrupt enable
    #define FUFIM	(1 << 1)	// FIFO underflow interrupt enable
//    LCDRIS_REG           =  0x20,    /* Raw Interrupt Status Register */
...
//    ClcdCrsrMIS_REG      = 0xc2c,    /* Cursor Masked Interrupt Status Register */
//
//    CLCDPeriphID0_REG    = 0xfe0,    /* (reset=0x11, 8-bit) Peripheral Identification Register0 */
//    CLCDPeriphID1_REG    = 0xfe4,    /* (reset=0x11, 8-bit) Peripheral Identification Register1 */
//    CLCDPeriphID2_REG    = 0xfe8,    /* (8-bit) Peripheral Identification Register2 */
//    CLCDPeriphID3_REG    = 0xfec,    /* (8-bit) Peripheral Identification Register3 */
//    CLCDPCellID0_REG     = 0xff0,    /* (reset=0x0d, 8-bit) PrimeCell Identification Register0 */
//    CLCDPCellID1_REG     = 0xff4,    /* (reset=0xf0, 8-bit) PrimeCell Identification Register1 */
//    CLCDPCellID2_REG     = 0xff8,    /* (reset=0x05, 8-bit) PrimeCell Identification Register2 */
//    CLCDPCellID3_REG     = 0xffc,    /* (reset=0xb1, 8-bit) PrimeCell Identification Register3 */

//====================================================

#define QVGA_320x240	(320  * 240)
#define VGA_640x480	(640  * 480)
#define XVGA_1024x768	(1024 * 768)

#define RBG555	2	// 16bpp, 5+5+5+1 = 16 bits
#define RBG565	2	// 16bpp, 5+5+5+1 = 16 bits
#define RBG888	3	// 24bpp, 8+8+8   = 24 bits
#define RBG8888	4	// 32bpp, 8+8+8+8 = 24 bits plus 8 dummy bits

//====================================================
// debug related definitions
enum cs752x_clcd_dbg_channels {
	cs752x_clcd_dbg_err   = (1 << 0),
	cs752x_clcd_dbg_debug = (1 << 1),
	cs752x_clcd_dbg_info  = (1 << 2),
	cs752x_clcd_dbg_irq   = (1 << 3),
	cs752x_clcd_dbg_sg    = (1 << 4),
	cs752x_clcd_dbg_dma   = (1 << 5),
	cs752x_clcd_dbg_pio   = (1 << 6),
	cs752x_clcd_dbg_fail  = (1 << 7),
	cs752x_clcd_dbg_conf  = (1 << 8),
};

static const int cs752x_clcd_dbgmap_err   = cs752x_clcd_dbg_fail;
static const int cs752x_clcd_dbgmap_info  = cs752x_clcd_dbg_info | cs752x_clcd_dbg_conf;
static const int cs752x_clcd_dbgmap_debug = cs752x_clcd_dbg_err  | cs752x_clcd_dbg_debug;

#if defined(CONFIG_FB_CS752X_CLCD_DEBUG)
#define cs752x_clcd_dbg(pdev, channels, args...)      \
    do {                                  \
    if (cs752x_clcd_dbgmap_err & channels) 		  \
        dev_err(&pdev->dev, args);  \
    else if (cs752x_clcd_dbgmap_info & channels)	  \
        dev_info(&pdev->dev, args); \
    else if (cs752x_clcd_dbgmap_debug & channels)	  \
        dev_dbg(&pdev->dev, args);  \
    } while (0)
#else
#define cs752x_clcd_dbg(pdev, channels, args...)
