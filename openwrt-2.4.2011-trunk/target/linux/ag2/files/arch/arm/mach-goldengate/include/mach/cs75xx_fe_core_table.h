...
typedef struct fe_lpm_entry_s {
	__u32		ip_addr[4];
	__u8		mask[4];	/* 6 bits mask each */
	__u8		priority[4];	/* 6 bits priority */
	__u16		result_idx[4];	/* 13 bits to result table */
} fe_lpm_entry_t;
...
#define FE_LPM_ENTRY_MAX			32	/* FIXME! confirm this number!! */
...
#define FE_MAX_ENTRY_SIZE			160	/* assume fe_xx_entry_t max size of is not over this */
