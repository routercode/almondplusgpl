...
#else
#define CONFIG_PCIE_NUM_PORTS 1 
static int num_port_sb_phy  = 1;
#endif
#endif
...
static void g2_intx_nop(unsigned int irq)
{
        return;
}

static void mask_intx_irq(unsigned int irq)
{
...
static void unmask_intx_irq(unsigned int irq)
{
	struct pcie_irq_resource *irqs = &pcie_irqs[irq - PCIE_IRQ_BASE];
        struct pcie_port *pp = bus_to_port(irqs->rc_bus_number);
...
static struct irq_chip g2_intx_chip = {
        .name = "G2-PCI-INTX",
        .ack = g2_intx_nop,
        .enable = unmask_intx_irq,
        .disable = mask_intx_irq,
        .mask = mask_intx_irq,
...
static void g2_msi_nop(unsigned int irq)
{
	return;
}
...
//debug_Aaron
	...
	set_irq_chip_and_handler(irq, &g2_msi_chip, handle_simple_irq);
...
//#endif
	...
	pp->res[0].start = GOLDENGATE_PCIE0_BASE + pp->rc_number * GOLDENGATE_PCI_MEM_SIZE + 0x5000;
	pp->res[0].end = pp->res[0].start + 0x4000 - 1;
	...
	pp->res[1].start = GOLDENGATE_PCIE0_BASE + pp->rc_number * GOLDENGATE_PCI_MEM_SIZE + 0x9000;
	pp->res[1].end = GOLDENGATE_PCIE0_BASE + pp->rc_number * GOLDENGATE_PCI_MEM_SIZE + 0x2000000-1;
...
g2_pcie_scan_bus(int nr, struct pci_sys_data *sys)
{
	...
	in_scanning_bus = 1;
	bus = pci_scan_bus(sys->busnr, &g2_pcie_ops, sys);
	in_scanning_bus = 0;
...
static int __init g2_pcie_map_irq(struct pci_dev *dev, u8 slot, u8 pin)
{
  ...
  //debug_Aaron, check where the request come from, device or RC ?
        ...
        if (isRC)
	{
        	...
        	ret = request_irq(IRQ_PCIE0 + pp->rc_number, &g2_pcie_irq, IRQF_SHARED, "G2 PCIE0 int", dev);
		...
		{
			set_irq_chip_and_handler(g2_pcie_map_no, &g2_intx_chip, handle_simple_irq);
//#ifdef PCIE_DEBUG
...
//debug_Aaron release POR
        ...
        */
        ...
        temp = readl(GLOBAL_GPIO_MUX_2);
        temp |= 0x10000000;
        writel(temp, GLOBAL_GPIO_MUX_2);

        /* per Alan Carr's suggestion write 0 first then wait then write 1 */
        temp = readl(PER_GPIO2_OUT);
        temp &= 0xEFFFFFFF;
        writel(temp, PER_GPIO2_OUT);

        temp = readl(PER_GPIO2_CFG);
        temp &= 0xEFFFFFFF;
        writel(temp, PER_GPIO2_CFG);
...
static int g2_pcie_probe(struct platform_device *pdev)
{
	struct resource *res;
...
#ifdef CONFIG_PM
static int g2_pcie_suspend(struct platform_device *pdev, pm_message_t state)
{
	int i;
	struct pcie_port *pp;

	printk("%s: state = %d, num_port_sb_phy=%d\r\n", __func__, state.event, num_port_sb_phy);
...
static int g2_pcie_resume(struct platform_device *pdev)
{
	int i;
	struct pcie_port *pp;

        printk("%s: num_port_sb_phy=%d\r\n", __func__, num_port_sb_phy);
...
static struct platform_driver g2_pcie_driver = {
	.probe		= g2_pcie_probe,
	.remove		= __devexit_p(g2_pcie_remove),
#ifdef CONFIG_PM
	//debug_Aaron for power management
