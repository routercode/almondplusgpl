...
#ifdef GPIO_FACTORY_BUTTON
/* Reset Button (Reboot or Restore Factory Default) */
#define LATCH_TIME		5
#define PUSH_VALUE		0
#define AUDIT_INTERVAL		100 // ms

static unsigned int gpio_fac_def;
static char gpio_fac_str[] = "GPIO_FAC_DEF";
static unsigned int audit_time;
static unsigned int audit_freq;
#endif
...
#ifdef GPIO_FACTORY_BUTTON
static irqreturn_t cs75xx_reset_gpio_handle(int irq, void *dev_instance)
{
	struct cs75xx_pwr *pwr = (struct cs75xx_cir *)dev_instance;
	unsigned char i = 0;

	dev_info(pwr->dev, "Restore Factory Default\n");
	wake_up(&pwc_wait_q);

	// count push time
	for (i = 0; i <= LATCH_TIME*audit_freq; i++) {
		if (gpio_get_value(gpio_fac_def) == PUSH_VALUE) {
			if ((i % audit_freq) == 0)
				printk("\r%d seconds to restore default!!\r", LATCH_TIME - i/audit_freq);
		}
		else {
			break;
		}
	...
	else if ((i/audit_freq) < LATCH_TIME) {
		Action = 0;
		printk("\nAbort!\n");
		//return;
...
static int cs75xx_reset_gpio_pin_init(void)
{
	audit_time = AUDIT_INTERVAL;
	audit_freq = 1000 / audit_time;

	if (gpio_request(gpio_fac_def, gpio_fac_str)) {
		printk("Can't get GPIO PIN %d for %s!\n", gpio_fac_def, gpio_fac_str);
		return -EPERM;
	}
	if (request_irq(gpio_to_irq(gpio_fac_def), cs75xx_reset_gpio_handle, IRQF_DISABLED | IRQ_TYPE_EDGE_FALLING, gpio_fac_str, pwr)) {
		printk("Can't request IRQ GPIO PIN %d for %s!\n", gpio_fac_def, gpio_fac_str);
		gpio_free(gpio_fac_def);
		return -EPERM;
	}
...
static int cs75xx_reset_gpio_pin_exit(void)
{
	free_irq(gpio_to_irq(gpio_fac_def), NULL);
	gpio_free(gpio_fac_def);
...
#endif
	dev_info(pwr->dev, "0x%08p: Power event by ", jiffies);
...
static int cs75xx_pwr_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
...
#ifdef GPIO_FACTORY_BUTTON
	gpio_fac_def = platform_get_irq_byname(pdev, "gpio_reset_btn");
	if (gpio_fac_def == -ENXIO) {
		dev_err(&pdev->dev, "Func: %s - can't get resource %s\n", __func__, "gpio_reset_btn");
		goto fail;
	}
	dev_err(&pdev->dev, "\tgpio_fac_def = %d\n", gpio_fac_def);
#endif
...
#ifdef GPIO_FACTORY_BUTTON
	if (cs75xx_reset_gpio_pin_init()) {
		free_irq(pwr->irq, NULL);
		goto fail;
	}
#endif
