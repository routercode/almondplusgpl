 ...
 * cs75xx_reg.c
 *
 * $Id: cs75xx_reg.c,v 1.1.1.1 2011/08/10 01:19:26 peebles Exp $
 *
...
int cs_reg_ioctl(struct file *file, unsigned long arg, unsigned int cmd)
{
	...
	case GMIIREG:
		...
		if ((phy_addr == 1) || (phy_addr == 2) || (phy_addr == 0))	{
			for (i = 0; i < phy_len ; i++)	{
				unsigned int data;
	...
	case SMIIREG:
		...
		if ((phy_addr_s == 1) || (phy_addr_s == 2) || (phy_addr_s == 0))	{
