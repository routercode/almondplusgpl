...
static void cs75xx_seed_gen(struct cs75xx_trng *trng)
{
		...
		*addr_RTC_RTCALM = 0x81; /* enable rtc alarm */
		...
		//printk ("rand_num , %08x ", rand_num);
		rand_num = ((rand_num << 8) & 0xFF00) + (readl(0xf0070044) & 0xFF);
		//printk ("rand_num , %04x ", rand_num);
...
static int cs75xx_hwrng_data_read(struct hwrng *rng, u32 *data)
{
	...
	//printk("1st value,%08x%08x%08x%08x\n", data[3],data[2],data[1],data[0]);
...
static int cs75xx_hwrng_init(struct hwrng *rng)
{
	...
	int rc = trng_init(&trng->hw, (uint32_t)trng->base, ELPTRNG_IRQ_PIN_DISABLE, ELPTRNG_NO_RESEED);
	//int rc = trng_init(&trng->hw, (uint32_t)trng->base, ELPTRNG_IRQ_PIN_DISABLE, ELPTRNG_RESEED);

	/**Need to reseed by nonce manually ***/
	/*uint32_t * addr;
	uint32_t  data;
	addr = trng->base;
	*addr = 0xc0000000;
...
#ifdef CONFIG_TRNG_DIAG_ON
	{
	extern int cs75xx_trng_diag(uint32_t, int32_t);
	cs75xx_trng_diag((uint32_t)trng->base, jiffies);
	}
#endif
