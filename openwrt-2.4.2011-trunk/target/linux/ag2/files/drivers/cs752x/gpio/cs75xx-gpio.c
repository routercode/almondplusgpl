...
static void cs75xx_gpio_ack_irq(unsigned int irq)
{
...
static void cs75xx_gpio_mask_irq(unsigned int irq)
{
	unsigned int gpio = irq_to_gpio(irq);
	void __iomem *base = cs75xx_gpio_base[gpio / GPIO_BANK_SIZE];
...
static void cs75xx_gpio_unmask_irq(unsigned int irq)
{
	unsigned int gpio = irq_to_gpio(irq);
	void __iomem *base = cs75xx_gpio_base[gpio / GPIO_BANK_SIZE];
...
static int cs75xx_gpio_set_irq_type(unsigned int irq, unsigned int type)
{
	unsigned int gpio = irq_to_gpio(irq);
...
static struct irq_chip cs75xx_gpio_irq_chip = {
	...
	.ack = cs75xx_gpio_ack_irq,
	.mask = cs75xx_gpio_mask_irq,
	.unmask = cs75xx_gpio_unmask_irq,
	.set_type = cs75xx_gpio_set_irq_type,
};
...
static int __devinit cs75xx_gpio_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < GPIO_BANK_NUM; i++) {
		...
		for (j = GPIO_IRQ_BASE + i * GPIO_BANK_SIZE;
			...
			set_irq_chip(j, &cs75xx_gpio_irq_chip);
			set_irq_handler(j, handle_edge_irq);
			set_irq_flags(j, IRQF_VALID);
		}

		set_irq_chained_handler(cs75xx_irq_gpio[i], cs75xx_gpio_irq_handler);
		set_irq_data(cs75xx_irq_gpio[i], (void *)i);
	}
...
static int __devexit cs75xx_gpio_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < GPIO_BANK_NUM; i++) {
		...
		for (j = GPIO_IRQ_BASE + i * GPIO_BANK_SIZE;
		     j < GPIO_IRQ_BASE + (i + 1) * GPIO_BANK_SIZE; j++) {
			set_irq_chip(j, NULL);
			set_irq_handler(j, NULL);
			set_irq_flags(j, 0);
		}

		set_irq_chained_handler(cs75xx_irq_gpio[i], NULL);
		set_irq_data(cs75xx_irq_gpio[i], NULL);
	}
