...
{
	return container_of(feed->demux, G2_TS_INFO_T, demux);
}
...
#ifdef TS_DEBUG
			printk("%s : TS RXQ %d buffer %i address = %x \n",
	...
	for (i=0; i<G2_TS_MAX_QUEUE_NUM; i++) {
		...
		while (rxq_wptr != rxq_rptr) {
			...
			if (curr_desc->word0.bits.own == CPU) {
				...
				tsdvb->demux.playing = i;/*indicate the RXQ ID*/
				dvb_dmx_swfilter_packets(&tsdvb->demux,
...
static int g2_ts_start_feed(struct dvb_demux_feed *f)
{
	...
	if(qid >= G2_TS_MAX_CHANNEL) { /* control channel */
		data_qid = qid - G2_TS_MAX_CHANNEL;
		channel = tsdvb->rxq_map[data_qid].channel;
		g2_ts_set_rxpid_enable(channel, CS_ENABLE);
		if (f->pid < 0x2000) {
			g2_ts_dmx_add_pid(data_qid, f->pid);
...
static int g2_ts_stop_feed(struct dvb_demux_feed *f)
{
	...
	filter = (struct dmxdev_filter *)f->feed.ts.priv;
	qid = filter->dev->dvbdev->id;
	/* printk("%s : pid=%d index=%d\n",__func__,f->pid,f->index); */
	/* printk("%s : dvbdev->id = %d\n",__func__,qid); */
...
static struct mxl241sf_config mxl241sf_g2_config = {
...
static int __devinit g2_ts_frontend_init(G2_TS_INFO_T *tsdvb)
{
	...
	tsdvb->frontend = dvb_attach(mxl241sf_attach, &mxl241sf_g2_config,
					tsdvb->i2c_adap);

/*	if (tsdvb->frontend) {
		tsdvb->frontend->ops.set_voltage = g2_ts_set_voltage;
		break;
	}
*/
	if (!tsdvb->frontend) {
		printk("Could not attach frontend.\n");
		return -ENODEV;
	}

	ret = dvb_register_frontend(&tsdvb->dvb_adapter, tsdvb->frontend);
	if (ret < 0) {
		printk("%s : Register frontend fail !\n",__func__);
		if (tsdvb->frontend->ops.release)
			tsdvb->frontend->ops.release(tsdvb->frontend);
		tsdvb->frontend = NULL;
		return ret;
	}
...
static int __devexit g2_ts_remove(struct platform_device *pdev)
{
	...
	struct dvb_demux 	*dvbdemux = &tsdvb->demux;
