 ...
 */
static uint8_t *cs752x_nand_fill_oob(struct nand_chip *chip, uint8_t *oob,
...
static int cs752x_nand_block_isbad(struct mtd_info *mtd, loff_t offs)
{
	...
	write_flash_ctrl_reg(FLASH_TYPE, NAND_FLASH_TYPE);
#endif
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	while(1) {
		...
		if (unlikely(oob))
			oob = cs752x_nand_fill_oob(chip, oob, ops);
			  ...
			  size_t *retlen, const uint8_t *buf)
{
	...
	ret = cs752x_nand_do_write_ops(mtd, to, &chip->ops);
 ...
 */
static int cs752x_nand_read_subpage(struct mtd_info *mtd, struct nand_chip *chip, uint32_t data_offs, uint32_t readlen, uint8_t *bufpoi)
{
		...
		if (realpage != chip->pagebuf || oob) {
			...
			if (unlikely(ops->mode == MTD_OOB_RAW))
			...
			else
				{
					...
					chip->ecc.read_page(mtd, chip, chip->buffers->databuf, page);
					memcpy( bufpoi, chip->buffers->databuf, mtd->writesize);
			...
			if (unlikely(oob)) {
				...
				if (ops->mode != MTD_OOB_RAW) {
					...
					if (toread) {
						oob = cs752x_nand_transfer_oob(chip,
							oob, ops, toread);
						oobreadlen -= toread;
					}
				} else
					buf = cs752x_nand_transfer_oob(chip, buf, ops, mtd->oobsize);
			}
		     ...
		     size_t *retlen, uint8_t *buf)
{
	...
	ret = cs752x_nand_do_read_ops(mtd, from, &chip->ops);
...
/* 	dma_ssp_rxq5_intsts.bf.rxq5_eof = 0; */
	...
	addr = (unsigned int *)((unsigned int)addr + (unsigned int)(cs752x_host->io_base) );
	...
	*(chip->oob_poi+ chip->ecc.layout->oobfree[0].offset +  chip->ecc.layout->oobfree[0].length)= 0;
	...
	addr = (unsigned int *) ((page << chip->page_shift) % SZ_128M);
	addr = (unsigned int *)((unsigned int)addr + mtd->writesize + (unsigned int)(cs752x_host->io_base) );
	...
	if(((unsigned int)addr) == ((unsigned int)(cs752x_host->io_base)+SZ_128M))
	{
			      ...
			      uint8_t *buf, int page)
{
	...
	**/
	...
	test = rx_desc[dma_rxq5_rptr.bf.index].buf_adr;
	/* set axi_bus_len = 8 */
	...
	**/
	...
	addr =  (unsigned int *)((page << chip->page_shift) % SZ_128M);
	addr = (unsigned int *)((unsigned int)addr + (unsigned int)(cs752x_host->io_base) +mtd->writesize);
...
static void fill_bch_oob_data( struct nand_chip *chip )
{
	...
	for( ; eccsteps; --eccsteps, addr += group_offset ) {
		...
		for( i= 0; eccpos != ecc_end_pos ; ++i ) {
			bch_gen00.wrd = read_flash_ctrl_reg( addr + reg_offset*i );

			for( j= 0;  j < 4 && eccpos != ecc_end_pos ; ++j ,++eccpos ) {
				chip->oob_poi[ *eccpos ] = ( (bch_gen00.wrd >> (j*8) ) & 0xff);
			}
		}
	}
	...
	for (i = 0 ; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		...
		for( j= 0 ; j < chip->ecc.bytes ; j+=4) {
			...
			for( k =0; k < 4 && eccpos != ecc_end_pos ; ++k, ++eccpos ) {
				bch_oob0.wrd |=  chip->oob_poi[ *eccpos ]  << ( 8*k ) ;
			}

			write_flash_ctrl_reg( FLASH_NF_BCH_OOB0 + j , bch_oob0.wrd);
		}
				...
				for(j=0  ;j<((bch_sts.bf.bchErrNum+1)/2);j++)
				{
					...
					if ( (j+1)*2 <= bch_sts.bf.bchErrNum ) {
						if( ((bch_err_loc01.bf.bchErrLoc1 & 0x1fff)>>3) < 0x200) {
							printk("pdata[%x] %x , ",((i/eccbytes)*chip->ecc.size + ((bch_err_loc01.bf.bchErrLoc1&0x1fff)>>3)), p[(bch_err_loc01.bf.bchErrLoc1&0x1fff)>>3]);
					...
					{
						printk("j: %x  ,pdata[%x] %x , ",j ,((i/eccbytes)*chip->ecc.size + ((bch_err_loc01.bf.bchErrLoc0&0x1fff)>>3)), p[ (bch_err_loc01.bf.bchErrLoc0&0x1fff)>>3]);
	...
	for (i = 0,j = 0; eccsteps; eccsteps--, i++, j += eccbytes)
	{
		...
		if(chip->ecc.size == 256)
		{
			...
			if(ecc_gen0.wrd != eccdata)
				printk(" i (%x)HWecc error ecc_gen0.wrd(%x)  eccdata (%x)!\n", i,ecc_gen0.wrd,eccdata);
		}
		else
		{
			memset(eccode, 0 ,32);
...
int cs752x_nand_scan_tail(struct mtd_info *mtd)
{
...
#endif
		{
			memset( &cs752x_nand_ecclayout, 0, sizeof( cs752x_nand_ecclayout));
			cs752x_nand_ecclayout.eccbytes= mtd->writesize/chip->ecc.size*chip->ecc.bytes;
			if( sizeof( cs752x_nand_ecclayout.eccpos) < 4*cs752x_nand_ecclayout.eccbytes ) {
			...
			memset( cs752x_nand_ecclayout.eccpos, 0, sizeof( cs752x_nand_ecclayout.eccpos));
			eccStartOffset= mtd->oobsize - cs752x_nand_ecclayout.eccbytes;
			for( i = 0 ; i < cs752x_nand_ecclayout.eccbytes; ++i) {
				if( ( i+ eccStartOffset ) == chip->badblockpos) {
					continue;
				}
				cs752x_nand_ecclayout.eccpos[ i ]=  i + eccStartOffset;
			}

			cs752x_nand_ecclayout.oobfree[0].offset = 2;
			cs752x_nand_ecclayout.oobfree[0].length = mtd->oobsize - cs752x_nand_ecclayout.eccbytes - cs752x_nand_ecclayout.oobfree[0].offset;
	...
	mtd->erase = cs752x_nand_erase;
	mtd->point = NULL;
	mtd->unpoint = NULL;
	mtd->read = cs752x_nand_read;
	mtd->write = cs752x_nand_write;
	mtd->read_oob = cs752x_nand_read_oob;
	mtd->write_oob = cs752x_nand_write_oob;
	mtd->sync = cs752x_nand_sync;
	mtd->lock = NULL;
	mtd->unlock = NULL;
	mtd->suspend = cs752x_nand_suspend;
	mtd->resume = cs752x_nand_resume;
	mtd->block_isbad = cs752x_nand_block_isbad;
	mtd->block_markbad = cs752x_nand_block_markbad;
			     ...
			     struct mtd_oob_ops *ops)
{
	...
	memset(chip->oob_poi, 0xff, mtd->oobsize);
	cs752x_nand_fill_oob(chip, ops->oobbuf, ops);
...
static int cs752x_nand_default_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (chip->options & NAND_USE_FLASH_BBT)
		...
		ret = cs752x_nand_do_write_oob(mtd, ofs, &chip->ops);
		cs752x_nand_release_device(mtd);
	}
	if (!ret)
...
static int cs752x_nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
	...
	{
		printk("page(%x): chip->buffers->databuf[0]:%x, chip->oob_poi[0]:%x\n",page,chip->buffers->databuf[0],chip->oob_poi[0]);
		return 1;
	}
	...
	if(chip->oob_poi[0] != 0xff || chip->buffers->databuf[0] != 0xff)
	{
		printk("page(%x): chip->buffers->databuf[0]:%x, chip->oob_poi[0]:%x\n",(page+0x79),chip->buffers->databuf[0],chip->oob_poi[0]);
		return 1;
	}
#else
...
static int cs752x_nand_dev_ready(struct mtd_info *mtd)
{
 ...
 RD_STATUS:
	...
	if (ready == 0xff)
	{
		...
		printk("--------->   %s : FLASH_STATUS : old_sts:%x, new_sts:%x \n",__func__,old_sts,new_sts);
		goto RD_STATUS;
	}
...
/////// middle debug : oob size not 4 bytes alignment
	if(mtd->oobsize%4)
		mtd->oobsize = mtd->oobsize - (mtd->oobsize%4);
///////
...
static int __devinit cs752x_nand_probe(struct platform_device *pdev)
{
	...
	cs752x_host->mtd->name = "cs752x-nand";
#ifdef CONFIG_MTD_PARTITIONS

	nr_partitions = parse_mtd_partitions(cs752x_host->mtd, probes, &mtd_parts, 0);
	if (nr_partitions > 0)
	  part_type = "command line";

	//nr_partitions = sizeof(cs752x_partition_info)/sizeof(struct mtd_partition );
	if (nr_partitions <= 0) {
	  mtd_parts = cs752x_partition_info;
	  nr_partitions = sizeof(cs752x_partition_info)/sizeof(struct mtd_partition );
	  part_type = "builtin";
	}

	if (nr_partitions > 0)
		err = add_mtd_partitions(cs752x_host->mtd, mtd_parts, nr_partitions);
	else
#endif
	err = add_mtd_device(cs752x_host->mtd);
	if (err)
		goto err_add;
	...
	memcpy(test_last_page, (cs752x_host->io_base+cs752x_host->mtd->writesize-4), (cs752x_host->mtd->oobsize+4));
	...
	memset(test_last_page, 0, 4096);
	memcpy(test_last_page, (cs752x_host->io_base+cs752x_host->mtd->writesize-3), (cs752x_host->mtd->oobsize+3));
	...
	write_flash_ctrl_reg(0xf00500a8, 7);
		memset(test_last_page, 0, 4096);
	memcpy(test_last_page, (cs752x_host->io_base+cs752x_host->mtd->writesize-2), (cs752x_host->mtd->oobsize+2));
	...
	write_flash_ctrl_reg(0xf00500a8, 7);
		memset(test_last_page, 0, 4096);
	memcpy(test_last_page, (cs752x_host->io_base+cs752x_host->mtd->writesize-1), (cs752x_host->mtd->oobsize+1));
	for(i=0;i<cs752x_host->mtd->oobsize;i++)
	{
	...
	write_flash_ctrl_reg(0xf00500a8, 7);
		memset(test_last_page, 0, 4096);
	memcpy(test_last_page, (cs752x_host->io_base+0x8000000-4), (cs752x_host->mtd->oobsize+4));
	for(i=0;i<cs752x_host->mtd->oobsize;i++)
	{
