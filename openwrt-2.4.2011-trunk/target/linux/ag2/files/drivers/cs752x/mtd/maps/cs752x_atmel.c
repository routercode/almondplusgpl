/*
 * $Id: cs752x_atmel.c,v 1.1.1.1 2011/08/10 01:19:26 peebles Exp $
 *
...
//#define AT_INDIRECT_ACCESS   /*indirect access */
/*#define  g_page_addr  AT45DB321_PAGE_SHIFT    /* 321 : shift 10  ; 642 : shift 11  */
//#define  g_chipen     SERIAL_FLASH_CHIP1_EN   /* atmel */
*/
...
int init_cs752x_maps(void)
{
	...
	nr_parts = parse_redboot_partitions(cs752x_at_mtd, &parsed_parts);
	if (nr_parts > 0)
		printk(KERN_NOTICE "Found RedBoot partition table.\n");
	else if (nr_parts < 0)
		printk(KERN_NOTICE "Error looking for RedBoot partitions.\n");
#else
	parsed_parts = cs752x_partitions;
	/*	parts = cs752x_partitions;*/
	/*	nr_parts = sizeof(cs752x_partitions)/sizeof(*parts);*/
	nr_parts = sizeof(cs752x_partitions)/sizeof(*parsed_parts);
#endif /* CONFIG_MTD_REDBOOT_PARTS */

	if (nr_parts > 0)
		add_mtd_partitions(cs752x_at_mtd, parsed_parts, nr_parts);
	else
		add_mtd_device(cs752x_at_mtd);
...
{
	if (parsed_parts)
		del_mtd_partitions(cs752x_at_mtd);
	else
		del_mtd_device(cs752x_at_mtd);
