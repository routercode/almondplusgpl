/*
 * $Id: cs752x_m25p80.c,v 1.1.1.1 2011/08/10 01:19:26 peebles Exp $
 *
...
#ifdef CONFIG_MTD_REDBOOT_PARTS
	nr_parts = parse_redboot_partitions(cs752x_st_mtd, &parsed_parts, 0);
...
#else
	parsed_parts = cs752x_m25p80_partitions;
	nr_parts = sizeof(cs752x_m25p80_partitions) / sizeof(*parsed_parts);
#endif				/* CONFIG_MTD_REDBOOT_PARTS */

	if (nr_parts > 0)
		add_mtd_partitions(cs752x_st_mtd, parsed_parts, nr_parts);
	else
		add_mtd_device(cs752x_st_mtd);
...
{
	if (parsed_parts)
	    del_mtd_partitions(cs752x_st_mtd);
	else
	    del_mtd_device(cs752x_st_mtd);
