...
int init_cs752x_s25fl128p(void)
{
	...
	nr_mtd_parts =
	    parse_mtd_partitions(cs752x_spansion_mtd, probes, &mtd_parts, 0);
	if (nr_mtd_parts > 0)
		part_type = "command line";

#endif
	if (nr_mtd_parts <= 0) {
		mtd_parts = cs752x_s25fl128p_partitions;
		nr_mtd_parts = ARRAY_SIZE(cs752x_s25fl128p_partitions);
		part_type = "builtin";
	}

	printk(KERN_INFO "Using %s partition table\n", part_type);
	add_mtd_partitions(cs752x_spansion_mtd, mtd_parts, nr_mtd_parts);
...
{
	if (mtd_parts)
		del_mtd_partitions(cs752x_spansion_mtd);
