...
int cs_fe_fwdrslt_del_by_idx(unsigned int idx)
{
	...
	ret = cs_fe_l2_result_dealloc(fwdrslt_entry.l2.mac_da_replace_en,
			fwdrslt_entry.l2.mac_sa_replace_en,
			fwdrslt_entry.l2.l2_index);
	if (ret != 0) {
//		printk("%s::unable to dealloc L2 idx %d\n",
