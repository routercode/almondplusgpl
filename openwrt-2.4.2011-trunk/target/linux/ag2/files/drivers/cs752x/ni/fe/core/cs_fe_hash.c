 ...
 * cs_fe_hash.c
 *
 * $Id: cs_fe_hash.c,v 1.1 2011/08/10 02:29:20 peebles Exp $
 *
...
static __u64 cs_fe_hash_used_mask[FE_HASH_STATUS_ENTRY_MAX];
...
extern void ni_dm_byte(cs_uint32 location, int length);

int cs_fe_hash_add_hash(__u32 crc32, __u16 crc16, __u8 mask_ptr,
		__u16 rslt_idx, u16 *return_idx)
{
	...
	unsigned long flags;
#if 0
	printk("%s:%d:crc32 = %x, crc16 = %x, mask_ptr = %d, rslt_idx = %d\n",
		__func__, __LINE__, crc32, crc16, mask_ptr, rslt_idx);
#endif
	/*
	...
	if ((ret[0] == FE_TABLE_EENTRYNOTRSVD) &&
		...
		if (new_idx != hw_idx) {
			printk("%s:%d:new_idx %x != request_idx %x\n", __func__,
					__LINE__, new_idx, hw_idx);
...
ADD_HASH_COMPLETE:
	...
	cs_fe_hash_used_mask[mask_addr] |= (((__u64)0x01) << mask_offset);
	mod_timer(&cs_fe_hash_timer_obj, jiffies + HASH_TIMER_PERIOD);
...
int cs_fe_hash_get_hash(unsigned int sw_idx, __u32 *crc32, __u16 *crc16,
		__u8 *mask_ptr, __u16 *rslt_idx)
{
...
static inline __u64 reverse_n(__u64 x, int bits)
{
	__u64 m = ~0;
	switch (bits) {
	case 64:
...
__u16 cs_fe_hash_keygen_crc16(unsigned char polynomial, unsigned char *buff,
		unsigned int size)
{
	__u16 crc16 = 0x3FFF;
	unsigned int i;
	...
	if (polynomial == CRC16_CCITT) crc16 = 0xFFFF;
	...
	default:
		for (i = 0; i < size; i++)
			crc16 = crctable_ccitt16[
				(crc16 ^ ((unsigned char)buff[i])) & 0xFFL]
			   	^ (crc16 >> 8);
		break;
	case CRC16_14_1:
		for (i = 0; i < size; i++)
			crc16= crctable_poly_1[
				(crc16 ^ (unsigned char)buff[i]) & 0xFFL]
				^ (crc16 >> 8);
		break;
	case CRC16_14_2:
		for (i = 0; i < size; i++)
			crc16= crctable_poly_2[
				(crc16 ^ (unsigned char)buff[i]) & 0xFFL]
				^ (crc16 >> 8);
		break;
	case CRC16_14_3:
		for (i = 0; i < size; i++)
			crc16= crctable_poly_3[
				(crc16 ^ (unsigned char)buff[i]) & 0xFFL]
				^ (crc16 >> 8);
		break;
    } /* end switch() */

	crc16 = (__u16)reverse_n((__u64)crc16, 16);
#else
...
__u32 cs_fe_hash_keygen_crc32(unsigned char *buff, unsigned int bitNumber)
{
...
#ifdef FE_HASH_BYTE_CRC
	unsigned char size = bitNumber / 8;
	for (i = 0; i < size; i++)
		crc32 = crctable_32[(crc32 ^ (unsigned char)buff[i]) & 0xFFL] ^
			(crc32 >> 8);
	crc32 = (__u32)reverse_n((__u64)crc32, 32);
#else
...
#endif
	return crc32;
} /* cs_fe_hash_keygen_crc32 */

static void cs_fe_hash_fill_fields(unsigned char *pDstBuf,
		unsigned char *pSrcBuf, unsigned short StartPos,
		unsigned short TotalWidth)
{
	...
	if (TotalWidth <= 8) {
		if (RemainBits >= TotalWidth)
            *pDstByte |= ((*pSrcByte) & HHBitMask[TotalWidth-1]) << StartBit;
		else {
		...
		while (TotalWidth != 0) {
            if(TotalWidth >= 8) MoveBitsCount = 8;
            else MoveBitsCount = TotalWidth;
	...
	return;
} /* cs_fe_hash_fill_fields */

static void cs_fe_hash_do_bits_table(fe_sw_hash_t *swhash,
		unsigned char *HHBuffer)
{
	memset(HHBuffer, 0, FE_HASH_HASH_BYTES);

	cs_fe_hash_fill_fields(HHBuffer,
			swhash->mac_da,
	...
	return;
} /* cs_fe_hash_do_bits_table */
#endif

#if 1	/* Enable if wants to debug */
/* For debugging purpose */
static void print_HHBuffer(unsigned char *HHBuffer)
{
	unsigned char *buf = HHBuffer;

	printk("\n\n");
	printk("****************** hwBitsHash: HHBuffer ****************\n");
	ni_dm_byte((__u32)buf, FE_HASH_HASH_BYTES);

#if 0
	for(i = 0; i < FE_HASH_HASH_BYTES; i++) {
		printk("%2.2x ", HHBuffer[i]);
		if ((i != 0) && (((i+1) % 16) == 0))
			printk("\n");
	}
#endif
...
#endif	/* Enable for debug */


#if 0
int cs_fe_hash_calc_crc(fe_sw_hash_t *swhash, __u32 *pCrc32, __u16 *pCrc16,
		unsigned char crc16_polynomial)
{
	unsigned char HHBuffer[FE_HASH_HASH_BYTES];

	if ((swhash == NULL) || (pCrc32 == NULL) || (pCrc16 == NULL))
		return FE_TABLE_ENULLPTR;

	cs_fe_hash_do_bits_table(swhash, HHBuffer);
	printk("\n**** %s::Dump hash bits!\n", __func__);
	print_HHBuffer(HHBuffer);
...
#if 1	// Sample Code
int cs_fe_hash_add_entry(fe_sw_hash_t *swhash, __u8 mask_ptr, __u16 rslt_idx,
		u16 *return_idx)
{
	__u32 crc32;
	__u16 crc16;
...
void cs_fe_hash_timer_func(unsigned long data)
{
	...
	for (i = 0; i < FE_HASH_STATUS_ENTRY_MAX; i++) {
		...
		if (cs_fe_hash_used_mask[i] != 0) {
			...
			tmp64 = value[0] | (((__u64)value[1]) << 32);
			...
			for (j = 0; (j < 64) && (tmp_dyn != 0); j++, tmp_dyn >>= 1) {
				if ((tmp_dyn & 1)) {
					...
					if (tmp64 & (((__u64)0x1) << j))
						cs_vhash_update_last_use(hw_idx, 1);
					else
						cs_vhash_update_last_use(hw_idx, 0);
#if 0
					if (tmp64 & (((__u64)0x1) << j))
						cs_vhash_update_last_use(hw_idx);
					else
						cs_vhash_update_not_use(hw_idx);
#endif
				}
			/*
...
next_time:
		//cs_fe_hash_timer_obj.expires = jiffies + (HASH_TIMER_PERIOD);
		mod_timer(&cs_fe_hash_timer_obj, jiffies + HASH_TIMER_PERIOD);
		//cs_fe_hash_timer_obj.expires = jiffies + (HASH_TIMER_PERIOD);
...
void fe_tbl_HashHash_Fill_Fields(unsigned char *pDstBuf, unsigned char *pSrcBuf, unsigned short StartPos, unsigned short TotalWidth)
{
    unsigned short  StartByte = (unsigned short)StartPos >> 3;
    unsigned short  StartBit  = (unsigned short)StartPos % 8;
    unsigned char   RemainBits= 0, MoveBitsCount = 0;
    ...
    if(TotalWidth <= 8)
    {
        ...
        }else
        {
            *pDstByte |= ((*pSrcByte) << StartBit);
            TotalWidth -= RemainBits;
            *(pDstByte + 1) |= (((*pSrcByte) >> RemainBits) & HHBitMask[TotalWidth-1]);
        }
    ...
    }else if(TotalWidth <= 64)
    {
        while(TotalWidth)
        {
            ...
            *pDstByte |= ((*pSrcByte) << StartBit);
            *(pDstByte + 1) |= (((*pSrcByte) >> RemainBits) & HHBitMask[TotalWidth-RemainBits-1]);
            TotalWidth -= MoveBitsCount;
    ...
    {// 128 bits
        ...
        {
            *pDstByte |= ((*pSrcByte) << StartBit);
            *(pDstByte + 1) |= (((*pSrcByte) >> RemainBits) & HHBitMask[StartBit]);
            pDstByte++;
            pSrcByte++;
        }
    }//end if(TotalWidth <= 8)
...
{
    memset(HHBuffer, 0, sizeof(HHBuffer));
...
/* BLOCK      : LPM                                          	*/
/* CHIP       : G2                                       	*/
cs_uint16 cs_fe_hash_keygen_crc16(cs_uint8 polynomial ,cs_uint8 *buff,cs_uint16 size)
/* INPUTS     : o polynomial                                   */
/*              o buff pointer                                 */
/*              o size                                         */
/* OUTPUTS    : ----     					*/
...
/****************************************************************/
{
	...
	default:
    	for ( i=0; i<size; i++) {
    		crc16 = crctable_ccitt16[(crc16 ^ ((unsigned char)buff[i])) & 0xFFL] ^ (crc16 >> 8);
	    }
        break;
    
    case CRC16_14_1:
    	for (i=0; i<size; i++) {
	        crc16= crctable_poly_1[(crc16 ^ (unsigned char)buff[i]) & 0xFFL] ^ (crc16 >> 8);
        }
        break;
    
    case CRC16_14_2:
    	for (i=0; i<size; i++) {
	        crc16= crctable_poly_2[(crc16 ^ (unsigned char)buff[i]) & 0xFFL] ^ (crc16 >> 8);
        }
        break;
    
    case CRC16_14_3:
    	for (i=0; i<size; i++) {
	        crc16= crctable_poly_3[(crc16 ^ (unsigned char)buff[i]) & 0xFFL] ^ (crc16 >> 8);
        }
        break;
    }//end switch()
...
/****************************************************************/
/* $rtn_hdr_start Calculate crc32 				*/
/* CATEGORY   : Device                                          */
/* ACCESS     : Public                                          */
/* BLOCK      : LPM                                          	*/
/* CHIP       : G2                                       	*/
cs_uint32 cs_fe_hash_keygen_crc32(cs_uint8 *buff, cs_uint16 bitNumber)
/* INPUTS     : o polynomial                                    */
/*              o buff pointer                                  */
/*              o size                                          */
/* OUTPUTS    : ----     					*/
...
/* DESCRIPTION:                                                 */
/* This API is used to calculate CRC16 based on given polynomial */
/* 0 : x^16+x^12+x^5+1 (also known as CCITT CRC-32)             */
/* 1 : x^14+x^4+1                                               */
/* 2 : x^14+x^8+x^5+x^4+1                                       */
/* 3 : x^14+x^11+x^9+x^8+x^5+x^4+1                              */
/* $rtn_hdr_end                                                 */
/****************************************************************/
{
	...
	//printk("************************* CRC Calcuate ***************************\n");
	//ni_dm_byte(buff, size);

#ifdef FE_HASH_BYTE_CRC
	unsigned char size = bitNumber/8 ;
	for(i = 0; i < size; i++)
	{
	    crc32= crctable_32[(crc32 ^ (unsigned char)buff[i]) & 0xFFL] ^ (crc32 >> 8);
	}
...
//cs_status fe_tbl_HashHash_cal_crc(fe_sw_hash_t *swhash, cs_uint32 *pCrc32, cs_uint16 *pCrc16, cs_uint8 crc16_polynomial)
cs_status cs_fe_hash_calc_crc(fe_sw_hash_t *swhash, cs_uint32 *pCrc32, cs_uint16
	*pCrc16, cs_uint8 crc16_polynomial)
{
    fe_tbl_HashHash_do_bits_table(swhash);

	//rdmsg print_sw_hash_key(swhash);
    //rdmsg print_HHBuffer();


    *pCrc32 = cs_fe_hash_keygen_crc32(HHBuffer, FE_HASH_HASH_BITS);
    *pCrc16 = cs_fe_hash_keygen_crc16(crc16_polynomial ,HHBuffer, sizeof(HHBuffer));
    return CS_OK;
