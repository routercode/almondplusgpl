 ...
 *
 * $Id: cs_fe_table_generic.c,v 1.1.1.1 2011/08/10 01:19:26 peebles Exp $
 *
...
{
	fe_table_entry_t *p_rslt = (fe_table_entry_t*)kmalloc(
			sizeof(fe_table_entry_t), GFP_ATOMIC);

	if (p_rslt == NULL)
		return NULL;
	atomic_set(&p_rslt->users, 0);
	p_rslt->p_entry = kmalloc(table_type_ptr->entry_size, GFP_ATOMIC);
	if (p_rslt->p_entry == NULL) {
		kfree(p_rslt);
		return NULL;
	}
	memset(p_rslt->p_entry, 0x0, table_type_ptr->entry_size);
