...
int cs_tm_bm_init(void)
{
	...
	voq_prof_mem.voq_max_depth = CS_TM_BM_DEF_VOQ_MAX_DEPTH;
	voq_profile_tbl[0].voq_min_depth = CS_TM_BM_DEF_VOQ_MIN_DEPTH;
	voq_profile_tbl[0].voq_max_depth = CS_TM_BM_DEF_VOQ_MAX_DEPTH;
	cs_tm_bm_set_voq_profile_mem(0, &voq_prof_mem);

	/* init all voq memory(point to default voq profile) */
	voq_mem.voq_profile = 0;
	/* FIXME, performance monitor disabled temporarily */
	...
	for(i = 0; i < CS_MAX_VOQ_NO; i++) {
		...
		/* FIXME!! dest port might be changed.. for CPU VOQ */
...
static unsigned char cal_prob_in_base_8(unsigned char drop_prob)
{
	...
	rslt_val = (unsigned char)(((unsigned long)drop_prob) * 8 / 100) - 1;
	if (rslt_val > 7)
		rslt_val = 7;
	return rslt_val;
} /* cal_prob_in_base_8 */
