...
static int g2_rtc_setpie(struct device *dev, int enabled)
{
	RTC_RTCIM_t		rtcim;

	pr_debug("%s: pie=%d\n", __func__, enabled);

	spin_lock_irq(&g2_rtc_pie_lock);

	rtcim.wrd = g2_rtc_readl(G2_RTC_RTCIM);
	...
	g2_rtc_writel(G2_RTC_RTCIM,rtcim.wrd,0xffffffff);

	spin_unlock_irq(&g2_rtc_pie_lock);
...
static int g2_rtc_setfreq(struct device *dev, int freq)
{
	RTC_RTCIM_t		rtcim;
...
static const struct rtc_class_ops g2_rtcops = {
	...
	.irq_set_freq		= g2_rtc_setfreq,
	.irq_set_state		= g2_rtc_setpie,
