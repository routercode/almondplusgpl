...
#define INIT_BUF_GAP	32
#define HIGH_BUF_GAP	48
#define LOW_BUF_GAP	32
	...
	while ((long)jiffies - (long)end_time < 0) { //while(1) {
		...
		if (spdif->ctrl.dma_mode == 0) {
			...
			if (i >= (SINAUDIOSAMPLES*2-1)) {
				i = 0;
				cs75xx_dma_spdif_tx_ptr(&spdif->ctrl.tx_write_ptr, &spdif->ctrl.tx_read_ptr);
				...
				if (spdif->ctrl.tx_en == 0) {
					...
					if (dma_wt_ptr >= spdif->ctrl.tx_read_ptr) {
						tx_diff_ptr = dma_wt_ptr - spdif->ctrl.tx_read_ptr;
					}
					else {
						tx_diff_ptr = dma_wt_ptr + spdif->ctrl.tx_buf_num - spdif->ctrl.tx_read_ptr;
					}
				...
				if (i >= (SINAUDIOSAMPLES*2-1)) {
					...
					if (spdif->ctrl.tx_en == 0) {
						...
						if (dma_wt_ptr >= spdif->ctrl.tx_read_ptr) {
							...
							tx_diff_ptr = dma_wt_ptr + spdif->ctrl.dam_tbuf_num - spdif->ctrl.tx_read_ptr;
						}
...
int cs75xx_spdif_diag_ac3(spdif_diag_cmd_t diag_cmd)	/* non-PCM */
{
	...
	CWda15_StartClock(&spdif->CWda15_Peripheral, 48000);
	CWda15_StartSpdif(&spdif->CWda15_Peripheral);
	CWda15_StartData(&spdif->CWda15_Peripheral);
...
static void spdif_dump(void)
{
	...
	printk("dma_txq_wptr = %d\n", spdif->ctrl.dma_txq_wptr);
	printk("dma_txq_rptr = %d\n", spdif->ctrl.dma_txq_rptr);
	...
	printk("tx_write_ptr = %d\n", spdif->ctrl.tx_write_ptr);
	printk("tx_read_ptr = %d\n", spdif->ctrl.tx_read_ptr);
	printk("tx dam owner\n");
	cs75xx_dump_owner();
	cs75xx_dump_size();
...
static void spdif_isr_handler(void)
{
	...
	if (spdif->ctrl.tx_write_ptr >= spdif->ctrl.tx_read_ptr) {
		tx_diff_ptr = spdif->ctrl.tx_write_ptr - spdif->ctrl.tx_read_ptr;
	}
	else {
		tx_diff_ptr = spdif->ctrl.tx_write_ptr + spdif->ctrl.dam_tbuf_num - spdif->ctrl.tx_read_ptr;
	}
	...
	if (spdif->ctrl.audio_type == AUDIO_WAV) {
		...
		for (i = 0; i < round; i++) {
			if ((spdif->ctrl.tx_data_size == 16) && (spdif->ctrl.preamble == 1)) {
				if (copy_from_user(spdif->ctrl.tx_buf_curr, buf, spdif->ctrl.tx_buf_size)) {
					printk("%s:%d - copy_from_user fail!\n", __func__, __LINE__);
				}
...
cs_status cs75xx_dma_spdif_tx_ptr(u16 *wt_ptr_p, u16 *rd_ptr_p)
{
	...
	spdif->ctrl.dma_txq_wptr = reg_txq6_wptr.bf.index;

	reg_txq6_rptr.wrd = cs75xx_dma_read_reg(CS75XX_DMA_SSP_TXQ6_RPTR);
	spdif->ctrl.dma_txq_rptr = reg_txq6_rptr.bf.index;

	*wt_ptr_p = spdif->ctrl.dma_txq_wptr;
	*rd_ptr_p = spdif->ctrl.dma_txq_rptr;
...
cs_status cs75xx_dma_spdif_tx_update(u16 tx_wt_ptr)
{
	...
	while (spdif->ctrl.dma_txq_wptr != tx_wt_ptr) {
		change = 1;

		dma_map_single(NULL, __va(spdif->ctrl.dma_tdesc_vaddr[spdif->ctrl.dma_txq_wptr].buf_addr), spdif->ctrl.dma_tbuf_size, DMA_TO_DEVICE);

		if ((spdif->ctrl.dma_tdesc_vaddr[spdif->ctrl.dma_txq_wptr].own == 0) && spdif->ctrl.tx_en) {
			printk("Can't Update Tx Write Pointer(dma_wt_ptr = %d, wptr = %d, own = 0, rptr = %d, len = 0x%x)\n",
			tx_wt_ptr, spdif->ctrl.dma_txq_wptr, spdif->ctrl.dma_txq_rptr, spdif->ctrl.wait_write_len);

			cs75xx_dump_owner();
		}

		spdif->ctrl.dma_tdesc_vaddr[spdif->ctrl.dma_txq_wptr].own = 0; // DMA
		spdif->ctrl.dma_tdesc_vaddr[spdif->ctrl.dma_txq_wptr].buf_size = spdif->ctrl.dma_tbuf_size;
		spdif->ctrl.dma_txq_wptr = (spdif->ctrl.dma_txq_wptr + 1) % spdif->ctrl.dam_tbuf_num;
	}
...
static irqreturn_t cs75xx_dma_spdif_irq_handler(int irq, void *dev_instance)
{
	...
	reg_dma_ssp_inten.bf.TXQ6_INTERRUPTi_en = 0;
	cs75xx_ssp_write_reg(CS75XX_DMA_SSP_INTENABLE_0, reg_dma_ssp_inten.wrd);
	...
	if ((reg_dma_ssp_int.bf.SSP0_INTi == 1) || (reg_dma_ssp_int.bf.SSP1_INTi == 1)) {
		...
		reg_ssp_int.wrd = cs75xx_ssp_read_reg(CS752X_SSP_INTERRUPT);
		if (reg_ssp_int.wrd & BIT(16))
			printk("SPDIF_INT(0x%08X)\n", CWda15_Dump_Interrupt(&spdif->CWda15_Peripheral));
		if (reg_ssp_int.wrd & BIT(17))
			printk("SPDIFFIFOFULL_INT\n");

		if ((reg_ssp_int.wrd & BIT(16)) || (reg_ssp_int.wrd & BIT(17))) {
			printk("CWda15 INT 0x%08X\n", CWda15_Clear_Interrupt(&spdif->CWda15_Peripheral));
		}

		cs75xx_ssp_write_reg(CS752X_SSP_INTERRUPT, reg_ssp_int.wrd);
	}

	/* DMA Operation */
	if (reg_dma_ssp_int.bf.TXQ6_INTERRUPTi == 1) {
		reg_txq6_int.wrd = cs75xx_dma_read_reg(CS75XX_DMA_SSP_TXQ6_INTERRUPT);
		tx = reg_txq6_int.bf.txq6_eof;	// BIT(0)
		if (reg_txq6_int.bf.txq6_empty) {
			printk("TXQ6_EMPTY\n");
...
int cs75xx_spdif_dam_cofnig(void)
{
	...
	dev_info(spdif->dev, "Function: %s - SPDIF\n", __func__);
...
static cs_status cs75xx_spdif_dma_init(struct cs75xx_spdif *spdif)
{
	...
	spdif->ctrl.dma_txq_wptr = 0;
	spdif->ctrl.dma_txq_rptr = 0;
	...
	spdif->ctrl.tx_write_ptr = 0;
	spdif->ctrl.tx_read_ptr = 0;
	...
	if (request_irq(spdif->irq_spdif, cs75xx_dma_spdif_irq_handler, IRQF_SHARED, "cs75xx-dma-spdif", spdif)) {
		dev_err(spdif->dev, "ERROR: can't register IRQ for DMA of SPDIF\n");
		goto fail;
	}
...
fail:
	free_irq(spdif->irq_spdif, spdif);
...
#ifdef CONFIG_DAC_REF_INTERNAL_CLK
int cs75xx_spdif_clock_start(unsigned int sample_rate)
{
	struct cs75xx_spdif *spdif = platform_get_drvdata(cs75xx_spdif_dev);
...
//	cs75xx_spdif_enable();
	CWda15_StartClock(&spdif->CWda15_Peripheral, sample_rate);
//	CWda15_StartSpdif(&spdif->CWda15_Peripheral);
//	CWda15_StartData(&spdif->CWda15_Peripheral);
