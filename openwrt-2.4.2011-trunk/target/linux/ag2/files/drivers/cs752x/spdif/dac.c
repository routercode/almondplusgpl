...
#define SSP_DF_24BITUB_LINEAR	6
#define SSP_DF_24BITUL_LINEAR	7
...
void cs4341_hw_init(void)
{
	...
	cs4341_reg_i2c_read(0x10, CS4341_MCLK, &data);
	printk("REG(%d): - (%x)\n", CS4341_MCLK, data);
	cs4341_reg_i2c_read(0x10, CS4341_MODE, &data);
	printk("REG(%d): - (%x)\n", CS4341_MODE, data);
	cs4341_reg_i2c_read(0x10, CS4341_TRANS, &data);
	printk("REG(%d): - (%x)\n", CS4341_TRANS, data);
	cs4341_reg_i2c_read(0x10, CS4341_CHA_VOL, &data);
	printk("REG(%d): - (%x)\n", CS4341_CHA_VOL, data);
	cs4341_reg_i2c_read(0x10, CS4341_CHB_VOL, &data);
	printk("REG(%d): - (%x)\n", CS4341_CHB_VOL, data);
	cs4341_reg_i2c_read(0x10, CS4341_TRANS, &data);
...
int ssp_i2s_interrupt(int tx_int, int rx_int)
{
	int diff = 0;
	unsigned short round, tx_write_ptr, tx_read_ptr;
	//printk("ssp_i2s_interrupt\n");

	if (tx_int) {
		cs75xx_dma_ssp_tx_ptr(ssp_ctrl.ssp_index, &tx_write_ptr, &tx_read_ptr);

		if (tx_write_ptr >= tx_read_ptr)
			diff = tx_write_ptr - tx_read_ptr;
		else
			diff = ssp_ctrl.buf_num - tx_read_ptr + tx_write_ptr;
		ssp_i2s.out_len += diff * ssp_ctrl.buf_size;

		if (dac_wait_en == 1 && (diff <= LOW_BUF_GAP)) {
			dac_wait_en = 0;
                                 ...
                                 size_t count, loff_t * ppos)
{
	...
	if (ssp_ctrl.ssp_en == 0) {
		...
		if (buf_gap >= HIGH_BUF_GAP) {
			dac_wait_en = 1;
			if (0 == interruptible_sleep_on_timeout(&ssp_wait_q, DMA_TIME_OUT)) {
				cs75xx_dma_ssp_tx_ptr(ssp_ctrl.ssp_index, &tx_write_ptr, &tx_read_ptr);
...
static int dac_ssp_init(void)
{
	...
	request_cs75xx_dma_ssp_irq(ssp_ctrl.ssp_index, ssp_i2s_interrupt);
